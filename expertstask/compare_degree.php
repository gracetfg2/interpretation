		...
		foreach($projects as $value)
		{
			...
			if ($stmt2 = mysqli_prepare($conn, "SELECT * FROM `Project` WHERE `ProjectID`=? ")) {
				...
				while ($current_project = $result->fetch_assoc()) {
					...
					$current_doc= $current_project['degreeOfChange_rate1'];
  ...
  <ul class="pagination">
     ...
     <?php 
		...
		foreach($projects as $value)
		{
			...
			//Both not selected
			if( !$value['better_rate1'] && !$value['degreeOfChange_rate1']) $current_class='indicator';
			//Both selected
			if( $value['better_rate1'] && $value['degreeOfChange_rate1'])$current_class='indicator finish';
